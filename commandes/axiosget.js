...
zokou({
  ...
  try {
    ...
    const data = response.data;
    const messageText = `
ᬑ *CASEYRHODES XMD HOLY BIBLE* ᬒ
  ...
  try {
    ...
    if (definition) {
      ...
      await zk.sendMessage(dest, {
        ...
        contextInfo: {
          externalAdReply: {
            title: "CASEYRHODES XMD DICTIONARY",
...
zokou({
  ...
  try {
    ...
    const replyText = "*Wait CASEYRHODES XMD is getting your pair code 💧✅...*";
...
*CASEYRHODES TECH Element Information:*
🚀 *Name:* ${data.name}
🚀 *Symbol:* ${data.symbol}
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "CASEYRHODES XMD ELEMENT INFORMATION",
...
zokou({
  ...
  if (!githubUsername) {
    return repondre("Give me a valid GitHub username like: github Caseyweb");
  }
...
`;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "CASEYRHODES XMD GITHUB USER INFO",
